...
const { list, create, single, update, patch, destroy, search } = require('../src');
...
describe('modelOptions scopes support', () => {
  ...
  beforeAll(async () => {
    ...
    await Item.bulkCreate([
      { external_id: 'laptop1', name: 'Gaming Laptop', category: 'electronics', status: 'active', tenant_id: 1, price: 1299.99, is_featured: true },
      { external_id: 'book1', name: 'JavaScript Guide', category: 'books', status: 'active', tenant_id: 1, price: 29.99, is_featured: false },
      { external_id: 'phone1', name: 'Smartphone', category: 'electronics', status: 'inactive', tenant_id: 2, price: 699.99, is_featured: true },
      { external_id: 'desk1', name: 'Office Desk', category: 'furniture', status: 'active', tenant_id: 1, price: 299.99, is_featured: false },
      { external_id: 'tablet1', name: 'Tablet Pro', category: 'electronics', status: 'active', tenant_id: 2, price: 799.99, is_featured: true },
    ]);
  });
  ...
  describe('list operation with scopes', () => {
    ...
    test('should apply multiple scopes from modelOptions', async () => {
      ...
      expect(response.body.data.every(item => item.status === 'active' && item.category === 'electronics')).toBe(true);
    });

    test('should apply parameterized scopes from modelOptions', async () => {
      app.use('/items', list(Item, {}, { 
        scopes: [
          { name: 'byTenant', args: [1] },
          'activeOnly'
        ]
      }));
      ...
      expect(response.body.data.every(item => item.status === 'active' && item.tenant_id === 1)).toBe(true);
    });
    ...
    test('should combine scopes with regular modelOptions', async () => {
      ...
      expect(Object.keys(response.body.data[0])).toEqual(expect.arrayContaining(['id', 'name', 'status']));
  ...
  describe('single operation with scopes', () => {
    test('should apply scopes when fetching single item', async () => {
      ...
      const inactiveItem = await Item.findOne({ where: { status: 'inactive' } });
    ...
    test('should apply multiple scopes when fetching single item', async () => {
      ...
      const laptopItem = await Item.findOne({ where: { external_id: 'laptop1' } });
      ...
      const tabletItem = await Item.findOne({ where: { external_id: 'tablet1' } });
  ...
  describe('update operation with scopes', () => {
    ...
    test('should apply multiple scopes for updates', async () => {
      ...
      const tenant2Item = await Item.findOne({ where: { tenant_id: 2, status: 'active' } });
      const response1 = await request(app)
      ...
      const tenant1Item = await Item.findOne({ where: { tenant_id: 1, status: 'active' } });
      const response2 = await request(app)
  ...
  describe('patch operation with scopes', () => {
    test('should apply scopes to limit which items can be patched', async () => {
      ...
      const nonFeaturedItem = await Item.findOne({ where: { is_featured: false } });
      const response1 = await request(app)
  ...
  describe('error handling', () => {
    test('should handle invalid scope names gracefully', async () => {
      app.use('/items', list(Item, {}, { 
        scopes: ['nonExistentScope'] 
      }));

      const response = await request(app).get('/items');
      // Should not crash, might return all items or handle gracefully
      expect(response.status).toBe(200);
      // The actual behavior depends on Sequelize's handling of invalid scopes
    });
    ...
    test('should apply scopes before pre hooks run', async () => {
      ...
      app.use('/items', list(Item, {
        pre: async (context) => {
          ...
          const result = await Item.findAndCountAll(context.req.apialize.options);
