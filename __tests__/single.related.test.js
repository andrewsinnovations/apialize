...
describe("single() with related models", () => {
  ...
  describe("basic related endpoints", () => {
    test("should create related endpoints for a single related model", async () => {
      ...
      await request(app)
        .post("/posts")
        .send({ title: "Jane's Post", content: "Jane's content", user_id: user2Id });
      ...
      expect(postsRes.body.data.every(post => post.user_id === userId)).toBe(true);
    });
    ...
    test("should create single related record endpoint", async () => {
      ...
      const singlePostRes = await request(app).get(`/users/${userId}/posts/${postId}`);
    ...
    test("should return 404 for single related record that doesn't belong to parent", async () => {
      ...
      const singlePostRes = await request(app).get(`/users/${user1Id}/posts/${postId}`);
  ...
  describe("custom configuration", () => {
    ...
    test("should support related model options", async () => {
      ...
      for (let i = 1; i <= 8; i++) {
        await request(app)
          .post("/posts")
          .send({ title: `Post ${i}`, content: `Content ${i}`, user_id: userId });
      }
  ...
  describe("nested related models", () => {
    test("should support nested related models (posts with comments)", async () => {
      ...
      expect(commentsRes.body.data.every(comment => comment.post_id === postId)).toBe(true);
    });
  });
  ...
  describe("model name to path conversion", () => {
    test("should convert PascalCase model names to snake_case and pluralize", async () => {
      ...
      const RelatedThing = sequelize.define("RelatedThing", {
        id: {
          type: DataTypes.INTEGER,
          primaryKey: true,
          autoIncrement: true,
      ...
      const thingsRes = await request(app).get(`/users/${userId}/related_things`);
