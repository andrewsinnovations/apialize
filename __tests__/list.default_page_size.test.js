...
describe('list operation: snake_case configuration backward compatibility', () => {
  let sequelize;
  ...
  async function setupTestApp(config) {
    ...
    app.use('/items', list(Item, config));
  ...
  test('should accept allow_ordering (snake_case)', async () => {
    sequelize = new Sequelize('sqlite::memory:', { logging: false });
    const Item = sequelize.define(
      'Item',
      {
