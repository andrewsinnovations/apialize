...
describe("mixed hooks: single functions and arrays together", () => {
  ...
  test("pre hooks can modify where clause to filter results", async () => {
    ...
    // Create test data
    const category1 = await Category.create({ name: "Electronics", description: "Electronic items" });
    const category2 = await Category.create({ name: "Books", description: "Book items" });
    
    await Item.create({ external_id: "item1", name: "Laptop", category_id: category1.id, status: "active", price: 999.99 });
    await Item.create({ external_id: "item2", name: "Mouse", category_id: category1.id, status: "inactive", price: 29.99 });
    await Item.create({ external_id: "item3", name: "Novel", category_id: category2.id, status: "active", price: 25.99 });
    ...
    expect(response.body.data.map(item => item.name).sort()).toEqual(["Laptop", "Novel"]);
    expect(response.body.meta.whereClauseModified).toBe(true);
    expect(response.body.meta.preResult).toEqual({ step: 2, whereModified: true });
  });
    ...
    app.use("/items", single(Item, {
      pre: [
        ...
        async (ctx) => {
          ...
          ctx.req.apialize.options.include[0].attributes = ["name", "description"];
  ...
  test("pre hooks can modify attributes clause to control returned fields", async () => {
    ...
    expect(response.body.preResult).toEqual({ step: 2, attributesModified: true });
  });
});