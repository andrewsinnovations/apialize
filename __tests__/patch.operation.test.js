...
async function build({ mountSingle = true, mountCreate = true, patchOptions = {}, modelOptions = {} } = {}) {
  ...
  if (mountSingle) app.use("/items", single(Item, { id_mapping: patchOptions.id_mapping || "id" }));
...
describe("patch operation: comprehensive options coverage", () => {
  ...
  test("partial update only changes provided fields and ignores id mapping field", async () => {
    ...
    const created = await request(app).post("/items").send({ external_id: "p-1", name: "Alpha", desc: "x", flag: false });
    expect(created.status).toBe(201);
    const id = created.body.id;

    // Attempt to patch name and include id/external_id in body; mapping field must be ignored
  const res = await request(app).patch(`/items/${id}`).send({ name: "Beta", id: 999, external_id: "nope" });
  ...
  test("id_mapping: external_id returns param id and updates partially", async () => {
    ...
    await request(app).post("/items").send({ external_id: "ext-1", name: "A", desc: "x" });

    const res = await request(app).patch(`/items/ext-1`).send({ desc: "patched" });
    expect(res.status).toBe(200);
  ...
  test("empty patch responds success when record exists, otherwise 404", async () => {
    ...
    sequelize = ctx.sequelize;
    const { Item, app } = ctx;
    const created = await request(app).post("/items").send({ external_id: "p-2", name: "X" });
    const id = created.body.id;
  ...
  test("ownership scoping via query filters: mismatch returns 404, match updates", async () => {
    ...
    sequelize = ctx.sequelize;
    const { Item, app } = ctx;
    const r1 = await request(app).post("/items").send({ external_id: "u1", name: "N1", user_id: 1 });
    const r2 = await request(app).post("/items").send({ external_id: "u2", name: "N2", user_id: 2 });

    const miss = await request(app).patch(`/items/${r2.body.id}?user_id=1`).send({ name: "no" });
    expect(miss.status).toBe(404);

    const ok = await request(app).patch(`/items/${r1.body.id}?user_id=1`).send({ name: "yes" });
    expect(ok.status).toBe(200);
  ...
  test("middleware can enforce tenant scoping and modify values", async () => {
    ...
    const ctx = await build({ patchOptions: { middleware: [scope, override] } });
    sequelize = ctx.sequelize;
    const { Item, app } = ctx;
  ...
  test("404 when record not found with custom id_mapping", async () => {
    ...
    const res = await request(app).patch(`/items/not-there`).send({ name: "nope" });
